...
retweet_connections = (
    df_retweet_users.groupby(["user_screen_name", "original_user_screen_name"])
    .size()
...
retweet_connections = retweet_connections[
    retweet_connections["original_user_screen_name"]
    != retweet_connections["user_screen_name"]
]

retweet_connections[retweet_connections["original_user_screen_name"] == "Bleu_Rochelle"]


retweet_graph = nx.from_pandas_edgelist(
    retweet_connections, "user_screen_name", "original_user_screen_name", "count"
)
...
nx.draw_networkx(
    ...
    pos=pos,
    nodelist=d2.keys(),
    node_size=[v * 30 for v in d2.values()],
    edge_color="b",
)
plt.show()
# -
...
nx.draw_networkx(
    Gc,
    pos=pos2,
    nodelist=top_nodes.keys(),
    node_size=[v * 50 for v in top_nodes.values()],
    edge_color="b",
)
...
df_most_retweeted_users = (
    df_tweets.groupby(["user_id"])["retweet_count"]
    .agg("sum")
    .sort_values(ascending=False)
    .reset_index()
    .head(15)
...
df_most_favorited_users_with_info = (
    ...
    .join(df_users.set_index("user_id"))
    .drop_duplicates()
...
df_most_favorited_users_with_info = (
    df_most_favorited_users_with_info.join(df_tweets_by_user.set_index("user_id"))
    .drop_duplicates()
    .sort_values("favorite_count", ascending=False)
)
df_most_retweeted_users_with_info = (
    df_most_retweeted_users_with_info.join(df_tweets_by_user.set_index("user_id"))
    .drop_duplicates()
    .sort_values("retweet_count", ascending=False)
)
...
def get_related_hashtags_by_username(name):
    d_name = df_aggretated[df_aggretated["Name"] == name][["#", "Used"]].sort_values(
...
def get_user_info(name):
    return df_users[df_users["screen_name"] == name][
...
df_retweet_users_top_account = df_retweet_users[
    df_retweet_users["original_user_screen_name"].isin(top_influencers_rt_names)
]
rt_influencers = df_retweet_users_top_account.groupby(
    ["user_screen_name", "original_user_screen_name"]
).size()
who_rt = (
    rt_influencers.groupby("user_screen_name")
    .size()
...
df_nodes_p = df_retweet_users[["original_user_id", "original_user_screen_name"]].rename(
    columns={
        "original_user_id": "user_id",
        "original_user_screen_name": "user_screen_name",
    }
)
df_retweet_users_nodes = pd.concat(
    [df_retweet_users[["user_id", "user_screen_name"]], df_nodes_p]
).drop_duplicates()

df_retweet_users_edges = df_retweet_users_edges.rename(
    columns={"user_id": "Source", "original_user_id": "Target"}
)
df_retweet_users_nodes = df_retweet_users_nodes.rename(
    columns={"user_id": "id", "user_screen_name": "label"}
)
...
tqdm.pandas()
df_users["processed_description"] = df_users["description"].progress_apply(
    spacy_tokenizer
)
...
lda = LatentDirichletAllocation(
    n_components=NUM_TOPICS, max_iter=10, learning_method="online", verbose=True
)
...
text = spacy_tokenizer(
    "Les gilets jaune tous unis contre Macron. Tous dans la rue jusqu'à la démission"
)
