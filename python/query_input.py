...
def get_related_hash_by(name):
	...
	df_aggretated[df_aggretated['Name'] == name][['#',"Used"]].sort_values("Used", ascending = False)

def main():

	read_all_csv(csv_folder)

	options = {'tweet_by': get_tweet_by(query_input),
			 	'related_hash_by':get_related_hash_by(query_input)}
	try:
		options[request]

	except KeyError: print("Error: " + request + " is not a recognized request. Available requests are: tweet_by, related_hash_by")
